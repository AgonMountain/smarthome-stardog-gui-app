...
from gui.config import TableEventKey
...
def set_human_fields(window, values, tab_data, data):
    if len(values[TableEventKey.TABLE_HUMAN.value]) == 1:
        ...
        room = human['is_located_in'] #data['room_list'].get(human['is_located_in'])
        update_human_fields(window, human['name'],
                            room, #['name'],
                            human_id) #human['name'])
        return human_id
    else:
...
def set_room_fields(window, values, tab_data, data):
    if len(values[TableEventKey.TABLE_ROOM.value]) == 1:
        ...
        update_room_fields(window, room['name'], room['type'],
                           room_id) #room['name'])
        return room_id
    else:
        return None
...
def set_device_fields(window, values, tab_data, data):
    if len(values[TableEventKey.TABLE_DEVICE.value]) == 1:
        ...
        room = device['is_located_in'] #data['room_list'].get(device['is_located_in'])
        update_device_fields(window, device['name'], device['type'],
                             room, #['name'],
...
def set_door_fields(window, values, tab_data, data):
    if len(values[TableEventKey.TABLE_DOOR.value]) == 1:
        ...
        if door['is_open'] == 'false':
            is_open = False
        is_door_of_a = door['is_door_of_A'] #data['room_list'].get(door['is_door_of_A'])['name']
        is_door_of_b = 'None'
        if door['is_door_of_B'] != 'None':
            is_door_of_b = door['is_door_of_B'] #data['room_list'].get(door['is_door_of_B'])['name']
        update_door_fields(window, door['name'], is_open, is_door_of_a, is_door_of_b,
                           door_id) #door['name'])
        return door_id
    else:
        return None
...
def set_window_fields(window, values, tab_data, data):
    if len(values[TableEventKey.TABLE_WINDOW.value]) == 1:
        ...
        if room_window['is_open'] == 'false':
            is_open = False
        is_window_of = room_window['is_window_of'] #data['room_list'].get(room_window['is_window_of'])['name']
        update_window_fields(window, room_window['name'], is_open, is_window_of,
                             window_id) #room_window['name'])
        return window_id
    else:
        return None
