...
def delete_human(name, is_located_in):
  human_id = 'human_' + get_unique_id()
  insert('DELETE DATA { <http://smarthome#' + human_id + '> ' +
                                  'rdf:type agn:Human ;' +
                                  'agn:is_human_located_in <http://smarthome#' + is_located_in + '>; ' +
                                  'agn:has_human_name "' + name + '". }')


def delete_device(name, type, is_located_in):
  device_id = 'device_' + get_unique_id()
  insert('DELETE DATA { <http://smarthome#' + device_id + '> ' +
                                  'rdf:type agn:' + type + ' ;' +
                                  'agn:is_device_located_in <http://smarthome#' + is_located_in + '>; ' +
                                  'agn:has_device_name "' + name + '". }')


def delete_device_type(name):
  insert('DELETE DATA { <http://smarthome#' + name + '> rdfs:subClassOf agn:Device . }')


def delete_room(name, type):
  room_id = 'room_' + get_unique_id()
  insert('DELETE DATA { <http://smarthome#' + room_id + '> ' +
                                  'rdf:type agn:' + type + ' ;' +
                                  'agn:has_room_name "' + name + '". }')


def delete_door(name, is_door_open, is_door_of_list):
  ...
  insert('DELETE DATA { <http://smarthome#' + door_id + '> ' +
                                  'rdf:type agn:Door ;' +
...
def delete_window(name, is_window_open, is_window_of_list):
